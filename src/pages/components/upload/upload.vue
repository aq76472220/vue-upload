...
<script>  
    export default {
        ...
        data() {
            return {
              uploader: null,  //当前这个图片的对象
              previewImgList:[], //显示的数组
              outImgList:[], //传出去的图片数据
            };
        },
        mounted() {  
        ...
        methods: {
            initWebUpload() { 
                ...
                this.uploader.on('uploadSuccess', (file, response) => { // 上传成功时候执行
                    ...
                    this.outImgList=this.outImgList.push('22')

                    this.$emit('success', ); //图片上传成功后返回的数据
                });
...
.ss-upload-ul {position: relative;  z-index: 10; overflow: hidden}
.ss-upload-ul li{float: left; position: relative; margin-right: 15px;}
